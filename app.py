...
try:
    ...
    if GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET:
        ...
        logger.info('[OK] Google OAuth registered')
    else:
        logger.warning('[WARNING] GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET not set')
except Exception as e:
    oauth = None
    logger.warning(f'[WARNING] authlib not available: {e}')
    logger.exception(e)
...
def index():
    # pass user info (if any) to the template so client JS can adapt the UI
...
def login():
    ...
    # Explicitly specify redirect_uri for Authlib
    redirect_uri = url_for('auth_callback', _external=True)
    logger.info(f"Redirecting to Google login with redirect_uri: {redirect_uri}")
...
def auth_callback():
    if not oauth:
        logger.error("[FAILED] OAuth object is None")
        return jsonify({'error': 'OAuth not configured on server'}), 500

    try:
        logger.info(f"[CALLBACK] Request args: {dict(request.args)}")
        logger.info("[CALLBACK] Calling authorize_access_token()...")

        token = oauth.google.authorize_access_token()

        logger.info(f"[SUCCESS] Token keys: {list(token.keys())}")
        logger.info(
            f"[TOKEN] access_token: {token.get('access_token')[:50] if token.get('access_token') else 'None'}...")
        logger.info(f"[TOKEN] token_type: {token.get('token_type')}")
        logger.info(f"[TOKEN] id_token: {token.get('id_token')[:50] if token.get('id_token') else 'None'}...")

    except Exception as e:
        logger.error(f"[FAILED] authorize_access_token() error: {type(e).__name__}: {e}", exc_info=True)
        import traceback
    ...
    # Strategy 1: Parse id_token if present
    if token.get('id_token'):
        try:
            logger.info("[STRATEGY-1] Attempting to parse id_token...")
            from authlib.oauth2.rfc7523 import parse_token
            import json
            import base64

            # Manually decode id_token (without verification for now)
            ...
            if len(parts) == 3:
                ...
                logger.info(f"[SUCCESS-STRATEGY-1] Parsed id_token: {user_info}")
            else:
                logger.warning(f"[FAILED-STRATEGY-1] Invalid id_token format: {len(parts)} parts")
        except Exception as e:
            logger.warning(f"[FAILED-STRATEGY-1] Could not parse id_token: {type(e).__name__}: {e}", exc_info=True)

    # Strategy 2: Call userinfo endpoint with access token
    if not user_info:
        try:
            logger.info("[STRATEGY-2] Fetching from userinfo endpoint...")
            userinfo_url = 'https://openidconnect.googleapis.com/v1/userinfo'
            headers = {'Authorization': f'Bearer {token.get("access_token")}'}

            resp = requests.get(userinfo_url, headers=headers)
            logger.info(f"[USERINFO] Status: {resp.status_code}")

            if resp.status_code == 200:
                user_info = resp.json()
                logger.info(f"[SUCCESS-STRATEGY-2] Got userinfo: {user_info}")
            else:
                logger.error(f"[FAILED-STRATEGY-2] Status {resp.status_code}: {resp.text}")
        except Exception as e:
            logger.error(f"[FAILED-STRATEGY-2] Userinfo request error: {type(e).__name__}: {e}", exc_info=True)

    if not user_info:
        logger.error("[FAILED] Could not get user info from any strategy")
    ...
    if not email or not sub:
        logger.error(f"[WARNING] Missing required fields. email={email}, sub={sub}")
    ...
    logger.info(f"[SUCCESS] User logged in: {email}")
    # Update user DB with last_seen (helps compute active users for admin dashboard)
    try:
        users = load_user_db()
        users[email] = {
            'email': email,
        ...
        logger.error(f"Error updating user_db after login: {e}")
...
def login_required(f):
    """Simple decorator to protect routes — returns 401 JSON when not authenticated."""

    @wraps(f)
    def decorated(*args, **kwargs):
        if 'user' not in session:
            # For API endpoints, return JSON 401 for unauthenticated users
...
@login_required
def me():
    # Return limited user info for debugging / client use
...
def save_summary():
    try:
        ...
        if video_id not in video_data:
            return jsonify({'success': False, 'error': 'video_id not found'}), 404

        user = session.get('user') or {}
        user_email = user.get('email', 'unknown')

        saved = video_data[video_id].setdefault('saved_by', {})
        user_list = saved.setdefault(user_email, [])
        entry = {
            'text': text,
        ...
        video_data[video_id]['saved_by'] = saved
        save_video_data(video_data)

        return jsonify({'success': True, 'message': 'Summary saved to your account', 'saved_count': len(user_list)})
    except Exception as e:
